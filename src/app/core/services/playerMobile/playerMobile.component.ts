...
import { PlayerMobileService } from './playerMobile.service';
...
@Component({
	selector: 'playerMobile',
	templateUrl: './playerMobile.component.html',
	animations: [
...
export class PlayerMobileComponent implements OnInit, OnDestroy, AfterViewInit {
	...
	ngOnInit() {
		this.playerMobileService.getData()
			.subscribe(res => {
				if (res) {
	...
	close(){
        ...
        this.window.scroll(this._previousScrollPosition.left, this._previousScrollPosition.top);
